...
function renderFunnelLanding(funnel) {
  ...
  const videoUrl = funnel.video_url || '';
  if (videoUrl.match(/(youtube\.com|youtu\.be)/i)) vType = 'youtube';
  else if (videoUrl.match(/vimeo\.com/i)) vType = 'vimeo';
  ...
  if (vType === "youtube") {
    ...
    playerHTML = `<div class="video-wrapper">
        <iframe id="vimeo-player" src="https://player.vimeo.com/video/${videoUrl.match(/vimeo\.com\/(\d+)/)?.[1] || videoUrl}"
...
<body class="theme-${funnel.theme || 'dark'}">
  ...
  <script>
    (function() {
      ...
      var STORAGE_KEY = 'funnel_unlocked_${funnel.id || 'default'}';
...
function renderFunnelForm(funnel) {
  ...
  try {
    ...
    console.warn('Error parseando form_fields:', e.message);
    parsedFields = [];
  }
  ...
  const fieldsHTML = parsedFields.map((f) => {
    ...
    if (f.type === "textarea") {
      ...
      const selectOpts = Array.isArray(f.options) ? f.options : [];
      const optionsHtml = selectOpts.map(o => {
        const val = escapeHtml(typeof o === 'string' ? o : (o.value || o.text || o.name || String(o)));
        const lbl = escapeHtml(typeof o === 'string' ? o : (o.label || o.text || o.name || String(o)));
