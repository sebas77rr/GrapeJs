...
function renderFunnelLanding(funnel) {
  ...
  const ctaText = funnel.cta_text || "┬íQuiero acceder!";
...
<body class="theme-${funnel.theme || 'dark'}">
  <div class="container">
    <div class="eyebrow">Ô£ª Contenido exclusivo</div>
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>${escapeHtml(funnel.title)} ÔÇö Agenda tu cita</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link href="https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet">
  <style>
    :root {
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ÔöÇÔöÇ SELECTED SUMMARY ÔöÇÔöÇ */
    .selection-summary {
...
<body class="theme-${funnel.theme || 'dark'}">
  <div class="card">
    ...
    <div class="view active" id="view-form">
      <div class="view-header">
        <div class="view-emoji">­ƒÄ»</div>
        <h1 class="view-title">┬íEst├ís a un paso!</h1>
    ...
    <div class="view" id="view-calendar">
      <button class="btn-back" id="backToForm">ÔåÉ Volver</button>
      <div class="view-header">
        <div class="view-emoji">­ƒôà</div>
        <h1 class="view-title">Elige tu fecha</h1>
        <p class="view-sub">Selecciona el d├¡a y horario disponible</p>
      </div>

      <div class="calendar-header">
        <button class="cal-nav" id="prevMonth">ÔÇ╣</button>
        <span class="cal-month" id="calMonth">Junio 2026</span>
        <button class="cal-nav" id="nextMonth">ÔÇ║</button>
      </div>
    ...
    <div class="view" id="view-success">
      <div class="success-view">
        ...
        <h2 class="success-title">┬íCita confirmada!</h2>
  ...
  (function() {
    ...
    // ÔöÇÔöÇ Init calendar with current month ÔöÇÔöÇ
    var now = new Date();
    ...
    function loadSlots(dateStr) {
      ...
      fetch('/api/crm/availability?date=' + dateStr)
        ...
        .then(function(data) {
          ...
          if (slots.length === 0) {
            container.innerHTML = '<div class="slots-empty">­ƒÿö No hay horarios disponibles para este d├¡a.<br>Por favor elige otra fecha.</div>';
            return;
          }
          renderSlots(slots);
        })
        .catch(function() {
          container.innerHTML = '<div class="slots-empty">ÔÜá´©Å Error cargando horarios. Intenta de nuevo.</div>';
        });
    }

    // ÔöÇÔöÇ Render slots grouped by morning/afternoon ÔöÇÔöÇ
    function renderSlots(slots) {
      ...
      var html = '';
      if (morning.length > 0) {
        html += '<div class="slots-section"><div class="slots-section-title">ÔÿÇ´©Å Por la ma├▒ana</div><div class="slots-grid">';
        morning.forEach(function(s) { html += slotBtn(s); });
        html += '</div></div>';
      }
      if (afternoon.length > 0) {
        html += '<div class="slots-section"><div class="slots-section-title">­ƒîà Por la tarde</div><div class="slots-grid">';
    ...
    function selectSlot(startDate, startTime, el) {
      ...
      var days = ['domingo','lunes','martes','mi├®rcoles','jueves','viernes','s├íbado'];
      var months = ['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'];
    ...
    // ÔöÇÔöÇ Confirm appointment ÔöÇÔöÇ
    function confirmAppointment() {
      ...
      .then(function() {
        ...
        document.getElementById('successDetail').innerHTML =
          '­ƒôà <strong>' + days[dateObj.getDay()] + ', ' + dateObj.getDate() + ' de ' + months[dateObj.getMonth()] + ' ' + parts[0] + '</strong><br>' +
          '­ƒòÉ <strong>' + selectedSlot.startTime + '</strong><br>' +
          'Ô£à Estado: <strong>Confirmada</strong>';
        showView('success');
        launchConfetti();
      })
      .catch(function() {
        btn.disabled = false; btn.textContent = 'Confirmar cita ÔåÆ';
        alert('Error al confirmar la cita. Intenta de nuevo.');
      });
    }

    // ÔöÇÔöÇ Form submit ÔöÇÔöÇ
    form.addEventListener('submit', function(e) {
      ...
      .catch(function(err) {
        ...
        submitBtn.disabled = false; submitBtn.textContent = 'Continuar ÔåÆ';
      });
    });
