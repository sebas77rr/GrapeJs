...
export default function GrapesJSEditor({ initialData, projectName, saveStatus, onSave }: any) {
  const containerRef = useRef<HTMLDivElement>(null);
  const { state, actions } = useEditor("gjs-editor", initialData);
    ...
    if (state.editor) {
      state.editor.Modal.open({
        ...
        <div style="padding: 20px;">
          ...
          ;" placeholder="Pega tu código HTML aquí"></textarea>
  ...
  const initializeEditor = () => {
    if (state.editor) {
      ...
      // Force a refresh after layout is completely painted to fix canvas sizing
      // Force a refresh after layout is completely painted to fix canvas sizing
      setTimeout(() => {
        if (state.editor) {
          state.editor.refresh();
    ...
    return (
      <div className="h-12 border-b border-slate-800 flex items-center px-3 justify-between bg-slate-900">
        <div className="flex items-center gap-4">
          <button onClick={() => window.dispatchEvent(new CustomEvent('kiuflow-back-to-landings'))} className="flex items-center text-white mr-4 hover:text-slate-300">
            <ArrowLeft className="w-5 h-5 mr-2" />
            <span className="font-bold text-xl">KiuFlow Builder</span>
          </button>
          <div className="h-5 w-px bg-slate-700" />
          <input
            key={projectName}
            type="text"
            className="bg-transparent border border-transparent outline-none text-zinc-300 text-sm font-medium focus:text-white hover:text-white hover:bg-white/5 focus:bg-white/10 focus:border-slate-700 rounded px-2 py-1 transition-all w-64"
            defaultValue={projectName || "Cargando..."}
            onBlur={(e) => {
              const val = e.target.value.trim();
              if (val && val !== projectName && onSave) {
                onSave(state.editor, val);
              }
            }}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
        ...
        <div className="flex items-center space-x-1.5">
          <Button
            variant="default"
            size="sm"
            className="h-8 bg-indigo-600 hover:bg-indigo-700 text-white mr-2"
            onClick={() => onSave && onSave(state.editor)}
          >
            💾 Guardar {saveStatus === "saving" ? "..." : ""}
          </Button>
          <Tooltip>
            ...
            <TooltipContent side="bottom">Bloques</TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">Plantillas</TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">Editar Código</TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">Importar HTML</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">Limpiar Lienzo</TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">Vista Previa</TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">
              {showSidebar ? "Ocultar Panel de Propiedades" : "Mostrar Panel de Propiedades"}
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
        ...
        <div className="h-10 flex items-center justify-between px-3">
          <div className="flex items-center space-x-3">
            ...
            <Tooltip>
              ...
              <TooltipContent side="bottom">
                {showResponsivePanel
                  ? "Ocultar Panel Responsive"
                  : "Mostrar Panel Responsive"}
              </TooltipContent>
            </Tooltip>
          </div>
        ...
        <Tabs defaultValue="style" className="h-full flex flex-col">
          <TabsList className="grid w-full grid-cols-3 h-10 bg-slate-900 rounded-none border-b border-slate-800">
            <Tooltip>
              ...
              <TooltipContent side="bottom">Estilos</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
              ...
              <TooltipContent side="bottom">Atributos</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <TabsTrigger
              ...
              <TooltipContent side="bottom">Interactividad</TooltipContent>
            </Tooltip>
            {/* <Tooltip>
              <TooltipTrigger asChild>
  ...
  const renderEmptySelectionMessage = (icon: string, type: string) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-400 text-center p-6">
        ...
        <h3 className="text-sm font-medium mb-2">Ningún Elemento Seleccionado</h3>
        <p className="text-xs">
