  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>Librería de Bloques</DialogTitle>
          <DialogDescription>
            Arrastra y suelta los bloques para construir tu página. Haz clic en un bloque para añadirlo.
            to the canvas.
          </DialogDescription>
        </DialogHeader>
...
function EmptyBlocksMessage() {
  return (
    <div className="col-span-full text-center py-8 text-muted-foreground">
      <p>No se encontraron bloques para tu búsqueda.</p>
    </div>
  );
}
    ...
    <div className="sticky bottom-0 bg-background p-3 border-t mt-4">
      <div className="flex justify-between items-center">
        <span className="text-xs text-muted-foreground">
          {selectedCount} bloque(s) seleccionado(s)
        </span>
        <Button size="sm" onClick={onAddSelectedBlocks}>
          Añadir Bloques Seleccionados
        </Button>
      </div>
    </div>
  );
